...
(defun tokyopromenade-post-article (body)
  ...
  (let ((url-request-method "POST")
    ...
    (url-retrieve (tokyopromenade-uri)
		  (lambda (status edit-buffer)
		    (if (re-search-forward tokyopromenade-post-success-regexp nil t)
			(progn (message (concat "Post done. id=" (match-string 1)))
			       (run-hook-with-args 'tokyopromenade-after-post-article-hooks
						   (concat (tokyopromenade-uri) "?id=" (match-string 1)))
